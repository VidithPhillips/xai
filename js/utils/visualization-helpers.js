...
class VisualizationHelpers {
    ...
    safeDataAccess(data, path, defaultValue = 0) {
        try {
    ...
    createSvgContainer(container, margin = {top: 40, right: 40, bottom: 40, left: 40}) {
    ...
    setupResizeHandler(containerId, updateFn) {
        const resizeHandler = () => {
            const container = document.getElementById(containerId);
            if (!container) return;
